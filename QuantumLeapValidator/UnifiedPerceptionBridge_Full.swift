...
import AVFoundation
import AudioToolbox
// import PythonKit // Temporarily disabled for build testing
...
@objc class UnifiedPerceptionBridge: NSObject, ObservableObject {
    ...
    @Published var sessionDuration: TimeInterval = 0    // Python integration (temporarily disabled)
    private var pythonModule: Any?
    ...
    func startSession(targetReps: Int = 10) {
        ...
        processingQueue.async { [weak self] in
            ...
            if self.pythonInitialized, let pipeline = self.pipeline {
                do {
                    try pipeline.start_session(target_reps: targetReps)
                    print("🧠 Unified perception session started with AI pipeline")
                } catch {
                    print("⚠️ Failed to start AI pipeline, using fallback mode: \(error)")
                }
            }
